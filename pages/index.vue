<template>

	<main class="flex flex-col md:flex-row justify-between mt-4 md:mt-10 leading-8">

		<SideMenu class="md:w-1/4" />

		<article class="px-4 md:mr-24 md:w-3/4">

			<div class="mb-10">
				<h2 class="font-semibold text-4xl">About</h2>
				<p class="mt-3">
					こーひーあーるという名前で活動しています。
					釣りや登山などの自然が好きです。心の病気があり人と関わることが苦手です。
					工業大学で材料や化学系を専攻していましたが就活に失敗して気づいたらWEB開発系の会社に入社していました。
					気まぐれで業務外にたまに何かを個人開発します。
				</p>
			</div>

			<div class="mb-10">

				<h2 class="font-semibold text-4xl">Work</h2>
				
				<h3 class="mt-3 text-xl">2019.07 ~ 嗜好・健康品 通販会社</h3>
				<p>
					システム開発を行なっている事業部で、LAMP系エンジニアとして携わる。
				</p>
				<ul class="list-disc list-inside">
					<li>関連会社のHPをWordPressで制作</li>
					<li>ソーシャルログイン機能を同社別ECサイトから新規ECサイトに移植</li>
					<li>クーポン機能を同社別ECサイトから新規ECサイトに移植</li>
					<li>商品レビュー機能を同社別ECサイトから新規ECサイトに移植</li>
					<li>法定停電時のオンプレミスサーバーの保守</li>
					<li>自社ECサイトのPHPバージョンアップ 5系→7系</li>
					<li>自社ECサイトのCodeIgniterバージョンアップ 2系→3系</li>
					<li>自社ECサイトにSeleniumを使ったE2Eテストを導入</li>
					<li>オンプレミスのDNSサーバーで公開しているホストゾーンをAWS Route53に移行</li>
					<li>新規BtoBtoCサイトの画像配信サーバーをAWS S3とCloudFrontで構築</li>
					<li>自社ECサイトのREST APIを新規実装</li>
					<li>自社ECサイトのREST API開発にPHPUnitを導入</li>
				</ul>

				<h3 class="mt-3 text-xl">2018.07 ~ 2019.06 無職</h3>
				<p>
					個人ゲーム開発に打ち込む
				</p>

				<h3 class="mt-3 text-xl">2015.04 ~ 2018.06 中堅システム開発会社</h3>
				<p>
					モバイルブラウザゲーム事業部の版権ソーシャルゲームを扱うチームで、LAMP系エンジニアとして携わる。
				</p>
				<ul class="list-disc list-inside">
					<li>既存のイベント・キャンペーンの改修における開発</li>
					<li>既存イベント・キャンペーンの運用 不具合時の補填作業など</li>
					<li>MySQLサーバーの負荷軽減 スロークエリや大量発行しているクエリの軽減</li>
					<li>トランザクション未実装のために起こる不整合データの軽減</li>
					<li>ゲームプランナー向けCMSの追加開発</li>
					<li>新規イベント・キャンペーンの開発</li>
					<li>カスタマーサポート業務 ユーザーデータ調査 エンドユーザーへの返信メール作成</li>
					<li>テスター外注管理の補佐</li>
				</ul>
			</div>

			<div class="mb-10">
				<h2 class="font-semibold text-4xl">Certification</h2>
				<ul class="list-disc list-inside">
					<li>AWS Certified Solutions Architect (2020/4 取得)</li>
					<li>PHP5初級技術者 (2017/5 取得)</li>
					<li>オープンソースデータベース技術者認定資格(OSS-DB) Silver (2015/8 取得)</li>
					<li>LPICレベル1 (2015/7 取得)</li>
				</ul>
			</div>

			<CustomFooter />
		</article>
	</main>	
</template>

<script>
export default {
	name: 'IndexPage',
}
</script>
